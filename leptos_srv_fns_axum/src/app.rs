...
use server_fn::{
    ...
    codec::{
        ...
        Rkyv, SerdeLite, StreamingText, TextStream,
    },
...
pub async fn add_row(text: String) -> Result<usize, ServerFnError> {
    ...
    println!("Adding row {text:?} to the database!");
    if nth_run % 3 == 2 {
...
#[component]
pub fn UsingCreateServerAction() -> impl IntoView {
    let input_ref = NodeRef::<Input>::new();
    ...
    view! {
        <h3>Using <code>create_server_action</code></h3>
        <p>
...
pub fn FileWatcher() -> impl IntoView {
    ...
    create_effect(move |_| {
        spawn_local(async move {
            let watched_files_res = watched_files().await;
            if watched_files_res.is_err() {
                println!(
                    "watched_files > Aborting due to '{}'.",
                    watched_files_res.unwrap_err()
                );
                return;
            }
...
#[server]
pub async fn ascii_uppercase(text: String) -> Result<String, ServerFnError<InvalidArgument>> {
    if text.len() < 5 {
